 ...
 * 
 * Adapted from the Madness 2026 repo for static GitHub Pages deployment.
 */
...
// Computed
C.REGION_H = 8 * C.GAME_H + 7 * C.R1_GAP;
C.SIDE_H   = 2 * C.REGION_H + C.REGION_GAP;
C.SVG_W    = 2 * C.MARGIN + C.COLS * C.STEP - C.CONN_W;
C.SVG_H    = C.HEADER_H + C.SIDE_H + 2 * C.MARGIN;
...
function colForRound(round, side) {
  if (side === 'left')  return round - 1;
  if (side === 'right') return C.COLS - round;
  return 5;
}
...
class BracketApp {
  ...
  async init() {
    this.showLoading(true, 'Loading bracket data…');
    try {
